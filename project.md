...
**MVPの追加確認事項（2026-09-22時点）**：現時点では **1ユーザーにつき子供は1人**という前提で進めます。将来的な複数人対応（1.節の構想）は残しますが、現在の投稿・記録フォームには子供選択UIを設けず、ログインユーザーの子供を自動で紐付けます。
...
- 材料（何を使って作ったか）用のカラムは別途設けず、`food_name`に含めて自由入力する運用とします（例：「野菜と鶏肉のうどん」）。
...
### 3.5 投稿作成フローについて（2026-09-22確定）

「投稿する」画面は、`posts`単体ではなく**`feeding_records`と`posts`を1つのフォーム・1回の送信で同時作成**する仕様とします。「投稿」と「食事記録」を別々の画面として使い分けるのではなく、保護者にとっては「今日の記録を残す」1つの操作として扱うためです。

**フォーム入力項目と保存先の対応：**

| フォーム項目 | 必須/任意 | 保存先 |
|---|---|---|
| 写真 | 必須 | `posts.photo_path` |
| 今日作ったもの（材料込みの自由入力） | 必須 | `feeding_records.food_name` |
| 食事のタイミング（朝食/昼食/夕食/おやつ） | 必須 | `feeding_records.meal_time` |
| どれくらい食べたか | 任意 | `feeding_records.amount` |
| ひとこと | 任意 | `posts.caption`（`feeding_records.memo`は使わず空のまま） |
| 投稿日時 | 必須 | `posts.posted_at`（日付部分を`feeding_records.fed_at`にも使用） |

`child_id`はフォームに含めず、ログインユーザーの子供（3.1節末尾の前提により1人）を自動で紐付けます。`reaction`は`none`固定、`is_first_time`は`false`固定とし、初回食材の自動判定（5節のObserverパターン）は将来対応とします。

実装上は、`feeding_records`の作成 → `posts`の作成（`feeding_record_id`で紐付け）の順にDBトランザクション内で行い、どちらかが失敗した場合は両方ロールバックします。
